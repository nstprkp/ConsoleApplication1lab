...
void made(struct people* person, int n, char c, int t, char const_find[])
{
	...
	while (check_type(ch, const_find) != 1) {
		while (!feof(ptr) && ch != c) {
			ch = fgetc(ptr);
		}
	}
...
void find_inf(struct people* person, char ch, int* n)
{
	...
	if (check_type(ch, name_find) == 1) {
		...
		if (t != 0) {
			int k = *n;
			add_person(person, k, t);
			char c = 'A'; t++;
			made(person, k, c, t, age_find);
			c = 'R'; t++;
			made(person, k, c, t, residence_find);
			c = 'N'; t++;
			made(person, k, c, t, money_find);
			(*n)++;
...
int comp_by_money(const void* typ1, const void* typ2) {
	const struct people* p1 = (const struct people*)typ1;
	const struct people* p2 = (const struct people*)typ2;
	return strcmp(p1->money, p2->money);
}
